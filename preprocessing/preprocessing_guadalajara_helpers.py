...
def get_sensor_uuids(row, k_closest,distorid):
    ...
    median_sensors = pd.read_csv("guadalajara_sensors.csv")
...
def load_mexico_participant_details(phase=1):
    details = pd.read_excel(gdl_logs_filepath, engine='openpyxl')
    if (phase == 1):
        details = details.iloc[0:9]
...
def loading_guadalajara_static():
    
    
    gdl_pd = pd.read_excel(gdl_logs_filepath, sheet_name="Guadalajara Walking", engine='openpyxl')
    ...
    for idx, row in gdl_pd.iterrows():
        ...
        suffix_filename = "_" + str(row["Date"])[:10] + "_guadalajara"
        
        
    CALIBRATE = False

    gdl_logs = load_mexico_participant_details()
    gdl_static_airspeck = pd.DataFrame()

    for idx, row in gdl_logs.iterrows():
        uuid = row['Sensor UUID']    

        frame = load_mexico_static_airspeck_file(uuid, project_name="MX", suffix_filename="guadalajara",
                                  upload_type='sd_card',
        ...
        gdl_static_airspeck = gdl_static_airspeck.append(frame)
        
    gdl_static_airspeck['timestamp'] = gdl_static_airspeck.timestamp.dt.round('S', 'NaT')
    gdl_static_airspeck = gdl_static_airspeck.set_index('timestamp')
    uncalibrated_gdl = pd.read_csv("uncalibrated_gdl1.csv")
    gdl_static_airspeck['humidity'] = uncalibrated_gdl['humidity'].values
    gdl_static_airspeck['temperature'] = uncalibrated_gdl['temperature'].values
        
    return gdl_static_airspeck


def preprocessing_guadalajara_static(unprocessed_dataset):
    
    gdl_static_df = unprocessed_dataset.filter(items=['timestamp','pm2_5','temperature','humidity','gpsLongitude', 'gpsLatitude', 'UUID'])
    gdl_static_df = gdl_static_df.reset_index()
    static_gdl = parse_dow_hod(gdl_static_df)
...
def loading_guadalajara_personal():
    ...
    gdl_pers_airspeck = pd.DataFrame()

    for idx, row in gdl_pd.iterrows():
        sid = row["Subject ID"]
        date = str(row["Date"])[:10]

        suffix_filename = "guadalajara_" + str(row["Date"])[:10]

        frame = load_mexico_personal_airspeck_file(sid, project_name="MX", suffix_filename=suffix_filename,
                                  upload_type='manual',is_minute_averaged=True)

        if frame is not None:
            frame['walk'] = sid+ "_" +date
        gdl_pers_airspeck = gdl_pers_airspeck.append(frame)
        
    gdl_pers_airspeck['timestamp'] = gdl_pers_airspeck.timestamp.dt.round('S', 'NaT')
    gdl_pers_airspeck = gdl_pers_airspeck.set_index('timestamp')
    
    return gdl_pers_airspeck

def preprocessing_guadalajara_personal(unprocessed_dataset, static_data):
